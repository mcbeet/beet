...
def toolchain_operation(ctx: click.Context, title: str = None):
    ...
    try:
        ...
        plugin = format_obj(exc.args[0])
        err(f"Couldn't import plugin {plugin}.", exc.__cause__)
    except PluginError as exc:
        plugin = format_obj(exc.args[0])
        err(f"Plugin {plugin} raised an exception.", exc.__cause__)
...
def watch(ctx: click.Context):
    """Watch the project directory and rebuild on file changes."""
...
def init(
    ...
    with toolchain_operation(ctx, "Initializing new project..."):
        if yes:
            name = name or Path(ctx.obj.initial_directory).name
            ...
            name = name or click.prompt(
                "Project name", Path(ctx.obj.initial_directory).name
            )
